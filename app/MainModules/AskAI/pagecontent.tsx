...
import { useAuth } from '@/context/AuthContext'
import Link from 'next/link'
import { ChevronLeft, History as HistoryIcon, Loader2, MoreVertical, Pencil, Trash2, Check, X as XIcon, Lock, Send } from 'lucide-react'
...
export default function AskAI() {
      ...
      return (
        <div key={m.id || i} className="flex justify-end mb-3">
          <div className="bg-blue-500/[0.14] rounded-tl-2xl rounded-tr-[4px] rounded-bl-2xl rounded-br-2xl px-3 py-2.5 max-w-[82%]">
            {lines.map((line: string, j: number) => (
              <p key={j} className="m-0 text-[13px] leading-relaxed text-white">
                {parseListItems(line)}
              </p>
            ))}
          </div>
        ...
        <div className="bg-white/[0.08] border border-blue-500/30 rounded-tl-[4px] rounded-tr-2xl rounded-bl-2xl rounded-br-2xl px-3 py-2.5 max-w-[82%]">
          {lines.map((line: string, j: number) => {
            ...
            return (
              <div
                key={j}
                className={`my-0.5 leading-relaxed ${isNote
                    ? 'text-[11px] text-white/50'
                    : isHeading
                      ? 'text-[15px] font-bold text-white'
                      : isNumbered
                        ? 'text-[13px] font-mono text-white'
  ...
  if (authLoading || aiLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full bg-[#0b0e17]">
  ...
  return (
    <div className="flex flex-col w-full bg-[#0b0e17] text-white overflow-hidden relative" style={{ height: 'calc(100dvh - 64px)' }}>
      {showLoginPrompt && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-gradient-to-br from-blue-500 to-blue-900 text-white px-6 py-3 rounded-full text-sm font-medium flex items-center gap-2.5 z-50 shadow-lg shadow-black/30 animate-[slideUp_0.3s_ease]">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
            <circle cx="12" cy="7" r="4" />
          </svg>
        ...
        {view === 'chat' && isAuthenticated && (
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              onClick={handleNewChat}
              title="New chat"
              className="text-[11px] font-bold text-blue-300 bg-white/[0.06] border border-blue-500/25 rounded-full px-2.5 py-1.5 cursor-pointer"
            >
              ✏️ New
            </button>
            <button
            ...
            history.map((h: AskAIHistorySession) => {
              ...
              if (isDeleting) {
                return (
                  <div
                    key={h.sessionId}
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl mb-1.5 border border-red-500/25 bg-red-500/[0.08]"
                  >
                    <div className="flex-1 min-w-0">
                      <p className="m-0 text-[12px] text-white/80 truncate">
                        Delete "{h.title}"?
                      </p>
                    </div>
                    <button
                      ...
                      onClick={() => setDeletingId(null)}
                      disabled={isBusy}
                      className="text-[11px] font-bold text-white/50 bg-white/[0.06] border border-white/10 rounded-full px-2.5 py-1 shrink-0 disabled:opacity-40"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      onClick={() => confirmDelete(h.sessionId)}
                      disabled={isBusy}
                      className="text-[11px] font-bold text-red-400 bg-red-500/15 border border-red-500/40 rounded-full px-2.5 py-1 shrink-0 disabled:opacity-40"
                    >
                      {isBusy ? <Loader2 size={12} className="animate-spin" /> : 'Delete'}
                    </button>
                  </div>
              ...
              return (
                <div
                  key={h.sessionId}
                  className={`relative w-full flex items-center gap-1.5 p-2.5 rounded-xl mb-1.5 border ${isActive ? 'bg-orange-500/[0.12] border-blue-500/40' : 'bg-transparent border-transparent hover:bg-white/[0.04]'
                    }`}
                >
                  <button
                    type="button"
                      ...
                      {isRenaming ? (
                        <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                          <input
                            ...
                            maxLength={60}
                            disabled={isBusy}
                            className="flex-1 min-w-0 bg-white/[0.08] border border-blue-500/40 rounded-md px-1.5 py-0.5 text-[13.5px] font-bold text-white outline-none disabled:opacity-50"
                          />
                          <button
                            type="button"
                            onClick={() => confirmRename(h.sessionId)}
                            disabled={isBusy || !renameValue.trim()}
                            className="w-6 h-6 rounded-full flex items-center justify-center bg-blue-500/20 border-none cursor-pointer text-blue-400 shrink-0 disabled:opacity-40"
                          >
                            {isBusy ? <Loader2 size={12} className="animate-spin" /> : <Check size={13} />}
                          </button>
                          <button
                  ...
                  {!isRenaming && (
                    <>
                      <span className="text-[10.5px] text-white/30 shrink-0">{h.dateLabel}</span>
                      <div className="relative shrink-0" ref={openMenuId === h.sessionId ? menuRef : undefined}>
                        <button
                          type="button"
                          onClick={e => {
                            e.stopPropagation()
                        ...
                        {openMenuId === h.sessionId && (
                          <div className="absolute right-0 top-[calc(100%+3px)] z-30 bg-[#1a1a24] border border-white/10 rounded-lg overflow-hidden min-w-[110px] shadow-[0_8px_24px_rgba(0,0,0,0.4)]">
                            <button
                              type="button"
                              onClick={() => openRenameFor(h)}
                              className="w-full flex items-center gap-1.5 text-left px-2.5 py-1.5 bg-transparent border-none cursor-pointer text-white text-[11px] font-semibold"
                            >
                              <Pencil size={12} />
                              Rename
                            </button>
                            <button
                              ...
                              className="w-full flex items-center gap-1.5 text-left px-2.5 py-1.5 bg-transparent border-none border-t border-white/[0.06] cursor-pointer text-red-400 text-[11px] font-semibold"
                            >
                              <Trash2 size={12} />
                              Delete
                            </button>
        ...
        <>
          ...
          {!hasMessages && (
            <div className="flex-1 min-h-0 overflow-y-auto p-3.5 flex flex-col gap-3">
              ...
              {isAuthenticated && (
                <div className="flex flex-wrap gap-1.5 pl-8">
                  ...
                  <button
                    ...
                    onClick={() => setSuggestions(pickSuggestions())}
                    title="Refresh suggestions"
                    className="w-6 h-6 rounded-full bg-white/[0.08] border border-blue-500/25 flex items-center justify-center text-blue-300"
                  >
              ...
              {!isAuthenticated && (
                <div className="flex items-center justify-center gap-2.5 bg-white/[0.05] rounded-2xl py-8 px-5 mt-2 text-white/40 text-sm">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                    <circle cx="12" cy="7" r="4" />
                  </svg>
          ...
          {hasMessages && (
            <div ref={messagesRef} className="flex-1 min-h-0 overflow-y-auto p-3.5 flex flex-col gap-0 [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
              ...
              {aiError && (
                <div className="flex items-start gap-2.5 text-[13px] bg-white/[0.05] border border-white/10 rounded-2xl px-4 py-3 mx-0 my-2">
          ...
          <div className="px-3 py-2.5 border-t border-white/[0.08] shrink-0">
            <div className="flex items-center gap-2 bg-white/5 border border-blue-500/30 rounded-full py-1.5 pl-3.5 pr-1.5">
              <input
                ...
                type="text"
                value={question}
                placeholder={listening ? 'Listening...' : isAuthenticated ? 'Ask about today\'s matches…' : 'Login to use AI Assistant'}
                onChange={e => setQuestion(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) askAI() }}
                disabled={!isAuthenticated || sending}
                maxLength={100}
                className="flex-1 bg-transparent border-none outline-none text-white text-[13.5px] placeholder-white/30 disabled:opacity-50"
              />
              <button
                onClick={() => askAI()}
                title="Send"
                disabled={!isAuthenticated || !question.trim() || sending}
                className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center border-none ${question.trim() && !sending ? 'cursor-pointer bg-gradient-to-br from-blue-500 to-blue-900' : 'cursor-default bg-white/[0.08]'
                  }`}
              >
                <Send size={14} className={question.trim() && !sending ? 'text-white' : 'text-white/30'} />
              </button>
            </div>
          </div>
