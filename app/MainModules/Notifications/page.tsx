...
//   return (
//     <div
//       className={`relative rounded-2xl border transition-all duration-300 ${notif.isRead
...
//         <div className="flex gap-3">
//           <div
//             className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${notif.isRead
//                 ? "bg-white/6 text-[#666]"
...
//               className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-medium transition-all ${notif.isRead
...
//               <span
//                 className={`font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-0.5 rounded-full border ${notif.isRead
...
//         <div className="flex gap-3">
//           <div
//             className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${notif.isRead ? "bg-white/6 text-[#666]" : "bg-[#e91e8c]/20 text-[#e91e8c]"
//               }`}
//           >
//             <HeartIcon size={20} />
//           </div>
//           <div className="flex-1 min-w-0 pr-4">
//             <span
//               className={`inline-block font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-0.5 rounded-full border mb-1 ${notif.isRead
//                   ? "border-white/10 bg-white/5 text-[#555]"
...
//         <div className="flex gap-3">
//           <div
//             className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${notif.isRead ? "bg-white/6 text-[#666]" : "bg-[#9333ea]/20 text-[#9333ea]"
//               }`}
//           >
//             <CommentIcon size={20} />
//           </div>
...
//   const userEmail = user?.email ?? null;
//   const userUid = user?.userId ?? (user as any)?.id ?? null;   // fall back to whichever is available
...
//                   className={`flex items-center gap-1.5 rounded-xl px-4 py-1.5 text-sm font-medium transition-all ${filter === key
//                       ? key === "audio"
...
//                   {count != null && count > 0 && (
//                     <span
//                       className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold text-white leading-none min-w-[18px] text-center ${key === "audio" ? "bg-[#4a9eff]" : "bg-[#e91e8c]"
//                         }`}
//                     >
//                       {count}
//                     </span>
...
// ── sf360-notifications schema notifications (Dolly + future ROAR types) ──
// These come from the new single-table schema: PK/SK/notification_type/read
// instead of the old id/type/isRead shape. We keep both id (derived) and
// SK (raw, needed for PATCH/DELETE) on every item.
type SchemaNotification = {
  id: string;
  SK: string;
  PK?: string;
  type: string; // mirrors notification_type for the dispatcher below
  ...
  priority?: string;
  isRead: boolean;
  createdAt: number; // derived from sent_at for timeAgo()
  sent_at?: string;
};
...
const SparkleIcon = ({ size = 20 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2l1.8 5.2L19 9l-5.2 1.8L12 16l-1.8-5.2L5 9l5.2-1.8L12 2zM19 14l.9 2.6L22.5 17.5l-2.6.9L19 21l-.9-2.6-2.6-.9 2.6-.9L19 14zM5 15l.7 2 2 .7-2 .7-.7 2-.7-2-2-.7 2-.7L5 15z" />
  </svg>
);
...
// Normalizes a raw notification item (either the old shape, already carrying
// isRead/id, or the new sf360-notifications shape carrying read/PK/SK) into
// a consistent Notification we can render. Always keeps SK (if present) so
// mutation calls (markRead/clear) can target the exact item.
function normalizeNotification(raw: any): Notification {
  const hasSchemaShape = typeof raw.notification_type === "string" || typeof raw.SK === "string";

  if (hasSchemaShape) {
    const createdAt = raw.sent_at ? new Date(raw.sent_at).getTime() : raw.createdAt ?? Date.now();
    return {
      id: raw.id ?? raw.SK,
      SK: raw.SK,
      PK: raw.PK,
      ...
      cta_target: raw.cta_target ?? null,
  ...
  // Old-shape notifications already have id/type/isRead — pass through as-is.
  return raw as Notification;
}

function isSchemaNotification(n: Notification): n is SchemaNotification {
  return (n as SchemaNotification).SK !== undefined;
}
...
// ─── Schema Notification Card (Dolly + future ROAR direct-trigger types) ──────
// Generic renderer for anything coming from the sf360-notifications schema:
// title/body/cta_label/cta_target. New notification_type values (mentions,
// reactions, room-going-live, etc.) all render through this card automatically
// — no new card needed per type unless you want bespoke styling later.

type SchemaCardProps = {
  notif: SchemaNotification;
  onMarkRead: () => void;
  onClear: () => void;
  onCtaClick: () => void;
  markLoading: boolean;
...
const SchemaNotificationCard = React.memo(function SchemaNotificationCard({
  notif,
  onMarkRead,
  onClear,
  onCtaClick,
      ...
      <div className="p-4">
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0 pr-4">
            ...
            <p className={`text-[12.5px] leading-snug ${notif.isRead ? "text-[#666]" : "text-[#bbb]"}`}>
              {notif.body}
            </p>
            {notif.cta_label && notif.cta_target && (
              <button
                onClick={onCtaClick}
                className={`mt-2 inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-medium transition-all ${notif.isRead
                    ? "border-white/10 bg-white/5 text-[#666] hover:border-white/20 hover:text-[#aaa]"
                    : "border-[#4a9eff]/35 bg-[#4a9eff]/12 text-[#4a9eff] hover:border-[#4a9eff]/55 hover:bg-[#4a9eff]/20"
                  }`}
              >
                {notif.cta_label} <ArrowRightIcon />
              </button>
            )}
            <p className="mt-1.5 text-[11px] text-[#444]">{timeAgo(notif.createdAt)}</p>
          </div>
...
type CardProps = {
  ...
  onBattleClick: () => void;
  onRoarPostClick: (postId: string) => void;
  onSchemaCtaClick: (ctaTarget: string) => void;
  markLoading: boolean;
  clearLoading: boolean;
};
...
const NotificationCard = React.memo(function NotificationCard(props: CardProps) {
  ...
  // sf360-notifications schema items (Dolly, and any future ROAR type that
  // hasn't gotten a bespoke card) — check this before the legacy type
  // switches below, since notif.type here mirrors notification_type
  // (e.g. "dolly.reply_ready") rather than the old string enum.
  if (isSchemaNotification(notif)) {
    return (
      <SchemaNotificationCard
        notif={notif}
        onMarkRead={onMarkRead}
        onClear={onClear}
        onCtaClick={() => notif.cta_target && props.onSchemaCtaClick(notif.cta_target)}
        markLoading={markLoading}
        clearLoading={clearLoading}
      />
    );
...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const normalized = (res.data.notifications || []).map(normalizeNotification);
  ...
  // ── Actions ────────────────────────────────────────────────────────────────
  // sf360-notifications items are addressed by SK (not id), and mutation
  // calls need userId/email so the backend can resolve the right PK
  // candidate (raw uid, actualUserId, or the sanitized-email fallback).
  // For legacy (pre-schema) notifications that only have `id`, we fall back
  // to sending id as sk — those items predate this schema and may age out
  // via TTL rather than needing a real migration.

  const skFor = (n: Notification) => (isSchemaNotification(n) ? n.SK : n.id);
  ...
  const clearOne = useCallback(async (n: Notification) => {
    const sk = skFor(n);
    setActionLoading(n.id + "__clear");
    try {
      await axios.delete("/api/notifications", {
        data: { userId: userUid, email: userEmail, sk },
      });
  ...
  const clearAll = useCallback(async () => {
    ...
    try {
      await axios.delete("/api/notifications", {
        data: { userId: userUid, email: userEmail, all: true },
      });
      setNotifications([]);
      window.dispatchEvent(new CustomEvent("sf360:notifications-updated"));
    } catch (err) {
  ...
  // cta_target for schema notifications is a deep-link like
  // "sf360://roar/rooms/<roomId>/dolly/<sessionId>". Dolly itself is rendered
  // inside HomePage (the lobby), keyed by sessionId — not on the ROAR room
  // page — so we route back to HomePage with query params it reads on mount
  // to reopen the panel at that exact session.
  const handleSchemaCtaClick = useCallback((ctaTarget: string) => {
    const match = ctaTarget.match(/rooms\/([^/]+)\/dolly\/([^/]+)/);
    if (match) {
      const [, , sessionId] = match;
      router.push(`/MainModules/HomePage?openDolly=1&dollySessionId=${sessionId}`);
      return;
    }
    // Fallback: no known route mapping yet for this cta_target shape.
    console.warn("[Notifications] Unhandled cta_target shape:", ctaTarget);
  }, [router]);
        ...
        {loading ? (
          ...
          displayed.map((notif) => (
            <NotificationCard
              ...
              onClear={() => clearOne(notif)}
              onBattleClick={() =>
                router.push(`/battles/${(notif as BattleNotification).battleId}`)
              }
              onRoarPostClick={handleRoarPostClick}
              onSchemaCtaClick={handleSchemaCtaClick}
              markLoading={actionLoading === notif.id}
              clearLoading={actionLoading === notif.id + "__clear"}
            />
          ))
