...
export function AIChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadRecentSession = useCallback(async () => {
    ...
    try {
      ...
      if (data.sessionId) {
        setSessionId(data.sessionId);
        // Trust the server's own insertion order — it already interleaves
        // each question with its answer. Re-sorting by `id` here mixed
        // numeric and lexicographic comparisons (ids aren't guaranteed to
        // be clean sequential integers), which reshuffled messages into
        // "all questions, then all answers" after a refresh.
        setMessages(data.messages || []);
      } else {
        setSessionId(null);
        setMessages([]);
