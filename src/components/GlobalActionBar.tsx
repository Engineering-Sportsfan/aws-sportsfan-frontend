...
export default function GlobalActionBar() {
  ...
  const isAllowed =
    !!userEmail &&
    CREATE_POST_ALLOWED_EMAILS.map((e) => e.toLowerCase()).includes(userEmail);
      ...
      <div className="fixed bottom-15 right-4 md:bottom-15 md:right-6 lg:bottom-15 lg:right-6 z-50">
        ...
        >
          ...
          <img
            ...
            className="text-white/70 group-hover:text-[#C9115F] w-5 h-5 md:w-6 md:h-6 lg:w-6 lg:h-6 transition-colors duration-200"
          />
        </button>
      </div>
